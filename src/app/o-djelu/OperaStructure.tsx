...
export default function OperaStructure({ acts, connectors, onActClick }: OperaStructureProps) {
    ...
    return (
        <div ref={containerRef} className="relative w-full max-w-5xl mx-auto" style={{'--progress': pathProgress} as React.CSSProperties}>
            ...
            <div className="absolute top-0 left-1/4 -translate-x-1/2 w-full h-full">
                <svg width="100%" height="100%" viewBox="0 0 200 1800" preserveAspectRatio="none" className="absolute top-0 left-0 w-full h-full">
                    <path d="M 100 0 V 1800" stroke="hsl(var(--secondary) / 0.3)" strokeWidth="2" strokeDasharray="10 10" />
                    <path d="M 100 0 V 1800" stroke="hsl(var(--secondary))" strokeWidth="2" strokeDasharray={`${1800}`} strokeDashoffset={`${1800 * (1 - pathProgress)}`} />
                </svg>
            ...
            <div className="absolute top-0 right-1/4 translate-x-1/2 w-full h-full">
                 <svg width="100%" height="100%" viewBox="0 0 200 1800" preserveAspectRatio="none" className="absolute top-0 left-0 w-full h-full">
                    <path d="M 100 1800 V 0" stroke="hsl(var(--primary) / 0.3)" strokeWidth="2" strokeDasharray="10 10" />
                    <path d="M 100 1800 V 0" stroke="hsl(var(--primary))" strokeWidth="2" strokeDasharray={`${1800}`} strokeDashoffset={`${1800 * (1 - pathProgress)}`}/>
                </svg>
            ...
            {acts.map((act, index) => (
                <React.Fragment key={act.id}>
                    <div className="relative flex justify-center items-center my-16 md:my-8" style={{minHeight: '150px'}}>
                        {/* Left Side (Hell) */}
                        <div className="absolute left-1/4 -translate-x-1/2 flex items-center">
                            <div className="w-6 h-6 rounded-full bg-black border-2 border-secondary z-10"></div>
                        </div>

                        {/* Right Side (Creation) */}
                        <div className="absolute right-1/4 translate-x-1/2 flex items-center">
